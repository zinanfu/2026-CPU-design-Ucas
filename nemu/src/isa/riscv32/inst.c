...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // jump
  INSTPAT("??????? ????? ????? ??? ????? 1101111", jal     , J, if (rd == 1) {ftrace_call(s->pc + imm, s->pc);} R(rd) = s->snpc; s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 1100111", jalr    , I, if (rd == 1) {ftrace_call((src1 + imm) & ~1, s->pc);} int rs1 = BITS(s->isa.inst, 19, 15); if ((rd == 0) && (imm == 0) && (rs1 == 1)) {ftrace_ret(s->pc);} R(rd) = s->snpc; s->dnpc = (src1 + imm) & ~1);
  // b
